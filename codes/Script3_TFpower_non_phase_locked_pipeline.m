...
for subj = 1:length(files_name)
    ...
    % compute ERP for single subject
    cfg            = [];
    cfg.keeptrials = 'no';
    ERP            = ft_timelockanalysis(cfg,data);

    % extract ERP from each trial
    for i = 1:length(data.trial)
        data.trial{1,i} = data.trial{1,i} - ERP.avg;
    end
    cfg                = [];                
    cfg.method         = 'wavelet'; 
    cfg.keeptrials     =  'no';
...
save tf_power_npl_2_80Hz.mat  TFRwave_VP TFRwave_NP TFRwave_CC

%% baseline norminalization
clc;clear 
load('G:\Research Data\1_Projects\2_Merge and composition\Data and analysis\data_analysis_revised\data\2_statistical_analysis\tf_power_npl_2_80Hz.mat')
restoredefaultpath;
...
cfg.subplotsize = [4 6];
cfg.toi         = -1:0.1:1;
ft_clusterplot(cfg, stat.sp.theta.VPvsNP);colorbar;clim([-5 5]);
ft_clusterplot(cfg, stat.sp.theta.VPvsCC);colorbar;clim([-8 8]);
ft_clusterplot(cfg, stat.sp.theta.NPvsCC);colorbar;clim([-8 8]);
%% alpha
...
save tf_power_npl_statistics_2_80Hz.mat stat
